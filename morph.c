...
static int do_init(void)
{
    ...
    // Открываем файл для чтения
        file = fopen(path, "r");
            ...
            if (fgets(path, sizeof(path), file) != NULL) {
                ...
                // Формируем строку с экранированными обратными слешами
                sprintf(searchdir, "%s", path);
    ...
    //sprintf(searchdir, "C:\\Users\\Acer\\WordFormCheckerApp\\dict");
    for (i = 1; i <= NUMPARTS; i++) {
