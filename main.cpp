...
int runAllTests() {
    ...
    status |= QTest::qExec(&test_compare);
    status |= QTest::qExec(&test_num);
    status |= QTest::qExec(&test_adj);
    status |= QTest::qExec(&test_noun);
    status |= QTest::qExec(&test_verb);
    status |= QTest::qExec(&test_compare_words);
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2 && strcmp(argv[1], "test_func") == 0) {
   ...
   //readFiles("C:\\Users\\Acer\\WordFormCheckerApp\\incorrect.txt", "C:\\Users\\Acer\\WordFormCheckerApp\\correct.txt", wrongSentence, correctSentence, posTags, errors);
    ...
    if (errors.fileProcessingResult != fileProcessingCodes::filesProcessed) {
        ...
        std::cout<<"Program correctly completed" << std::endl;
    }
